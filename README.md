...
- ~~**Mail hosting**~~: done 6 August 2026. `dmytro@swiftsecured.com` is a real mailbox on **Zoho Mail Lite, EU data centre**, and sending and receiving are both verified. MX is `mx/mx2/mx3.zoho.eu` (10/20/50), SPF is `v=spf1 include:zohomail.eu ~all`, and DKIM signs with selector `zmail`. A test to Gmail landed in the inbox with `SPF: PASS` and `DKIM: PASS`, both aligned to the domain. The free tier was not usable: it is webmail-only and, more decisively, **not offered in the EU data centre at all**. Note Zoho Mail Lite has no IMAP/SMTP, so outreach is sent by hand from Zoho webmail — that is fine for a wave of 21 and becomes the reason to upgrade long before the ~800 first touches `outreach-channels.md` plans for.
- **DMARC is still missing.** SPF and DKIM pass, but with no `_dmarc` record Gmail reports `DMARC: FAIL`, meaning "no policy published" rather than an authentication failure. Add `_dmarc` TXT = `v=DMARC1; p=none; rua=mailto:dmytro@swiftsecured.com`. Start at `p=none` and only tighten to `quarantine` then `reject` after weeks of clean sending — a strict policy published before the setup is proven would reject our own outreach.
- **Apply form — works locally, still dead in production.** `/apply/` POSTs to `PUBLIC_FORM_ENDPOINT` and only reports success on a 2xx. `PUBLIC_CONTACT_EMAIL` is set in the local `.env` to `dmytro@swiftsecured.com`, and a local build renders a working `mailto:` on all 19 apply pages. **The deployed site still has neither value.** Set them in the **Cloudflare dashboard → the `swift-secure` Worker → Settings → Variables and Secrets**, which is where the production build reads them — see the table in `.cloudflare-deploy.md`. **The GitHub Actions repository variables are not the same thing and will not fix production**: since the move to Cloudflare, `.github/workflows/deploy.yml` builds only the noindexed Pages mirror, so setting them there fixes the fallback copy and nothing a customer sees. `PUBLIC_CONTACT_EMAIL` alone is enough to make the page usable; the endpoint wants a form-to-email service such as Formspree or Web3Forms. `.env` is gitignored, so it does not travel with the repo — copy `.env.example` on any new machine.
